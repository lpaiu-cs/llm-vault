# ltm-vault Setup Guide

이 문서는 `ltm-vault`를 로컬에서 인덱싱하고 Antigravity / Cursor / Claude Desktop 같은 MCP 클라이언트의 장기 메모리(LTM)로 연결하는 설치 기준 문서입니다.
...
```json
{
  "mcpServers": {
    "ltm-vault": {
      "command": "python3",
      "args": ["/absolute/path/to/ltm-vault/90_Engine/mcp_server.py"],
      "env": {
        "VAULT_ROOT": "/absolute/path/to/ltm-vault",
        "VAULT_DB": "/absolute/path/to/ltm-vault/90_Engine/ltm_cache.db",
...
```toml
[mcp_servers.ltm-vault]
command = "<REPO>/.venv/bin/python"
args = ["<REPO>/90_Engine/mcp_server.py"]

[mcp_servers.ltm-vault.env]
VAULT_ROOT = "<REPO>"
...
```bash
claude mcp add ltm-vault -- <REPO>/.venv/bin/python <REPO>/90_Engine/mcp_server.py
...
>
> ```jsonc
> "command": "C:\\Users\\me\\ltm-vault-private\\.venv\\Scripts\\python.exe",
> "args": ["C:\\Users\\me\\ltm-vault-private\\90_Engine\\mcp_server.py"],
> ```
...
```bash
git clone <repo-url> && cd ltm-vault
...
```text
ltm-vault의 vault_stats를 호출해서 현재 그래프 상태를 알려줘.
```
...
- **public `ltm-vault` (`upstream`) = 프레임워크 템플릿** — 누구나 scratch에서 시작할 수
  있는 골격입니다. 담는 것: 엔진(`90_Engine/`) + 정책(`00_System/`) + 문서/스크립트 +
  **빈 vault 스켈레톤**(지식 계층은 `README/.gitkeep`만) + `examples/mini-vault/`(데모).
  실제 지식 코퍼스는 담지 않습니다.
- **private `ltm-vault-private` (`origin`) = 실제 second brain 인스턴스** — 당신의 진짜
...
> `git worktree add -b public-main ../ltm-vault-public upstream/main` 후 거기서 스켈레톤/예시를
...
*macOS (launchd)* — [`scripts/launchd/com.ltm-vault-daemon.plist.example`](scripts/launchd/com.ltm-vault-daemon.plist.example):
...
*Linux (systemd user)* — [`scripts/systemd/ltm-vault-daemon.service.example`](scripts/systemd/ltm-vault-daemon.service.example):

```bash
mkdir -p ~/.config/systemd/user
cp scripts/systemd/ltm-vault-daemon.service.example ~/.config/systemd/user/ltm-vault-daemon.service
# <REPO> 치환 후:
systemctl --user daemon-reload && systemctl --user enable --now ltm-vault-daemon.service
...
'start "" "<REPO>\.venv\Scripts\pythonw.exe" "<REPO>\90_Engine\vault_daemon.py"' |
  Out-File -Encoding ascii "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\ltm-vault-daemon.cmd"
```
